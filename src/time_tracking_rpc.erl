...
handle_cast({#'basic.deliver'{delivery_tag = Tag}, #amqp_msg{payload = Payload}}, State = #{channel := Chan}) ->
    {ok, Json} = jsx:decode(Payload, [return_maps]),
    Response = handle_request(maps:get(<<"method">>, Json), maps:get(<<"params">>, Json)),
    amqp_channel:cast(Chan, #'basic.publish'{routing_key = maps:get(<<"reply_to">>, Json)}, #amqp_msg{payload = jsx:encode(Response)}),
